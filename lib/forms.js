...
Forms.set = function (doc, name, value) {
  ...
  var child = doc;
  while (remainder) {
    token = remainder.match(propertyAccessTokenRegex);
    token = token && token[0];
    if (token) {
      // returns only 2 results because of the missing /g flag
      parts = remainder.split(propertyAccessTokenRegex);
      name = parts[0];
      remainder = parts[1];
    } else {
      name = remainder;
