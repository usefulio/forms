...
Forms.change = function (e, tmpl, change) {
  var propertyName = e.currentTarget.name;
  var propertyValue = e.currentTarget.value;

  if (!_.isUndefined(change)) {
    change.propertyName && (propertyName = change.propertyName);
    change.propertyValue && (propertyValue = change.propertyValue);
  }
...
Forms.submit = function (e, tmpl, customDoc) {
  ...
  if (isValid)
    ...
    //   , errors: errors
    // });
    // jquery trigger accepts an Object or Array parameter http://api.jquery.com/trigger/
...
Forms.events = {
  'change, propertyChange': function (e, tmpl, change) {
    ...
    Forms.change(e, tmpl, change);
  }
  , 'submit': function (e, tmpl, doc) {
    if (e.isDefaultPrevented())
      return;
