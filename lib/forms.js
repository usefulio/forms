...
Forms.events = {
  'change, propertyChange': function (e, tmpl) {
    // Check if either defaultPrevent or if the submit event is manually disabled
    if (e.isDefaultPrevented() || ! Forms.changeEventIsActive(tmpl))
      return;

    console.log("Forms change event called. toggle: "+Forms.changeEventIsActive(tmpl));
  ...
  , 'submit form': function (e, tmpl) {
    // Check if either defaultPrevent or if the submit event is manually disabled
    if (e.isDefaultPrevented() || ! Forms.submitEventIsActive(tmpl))
      return;

    console.log("Forms submit event called. toggle: "+Forms.submitEventIsActive(tmpl));
...
Forms.submitEventIsActive = function (templateInstance, isActive) {
  check(templateInstance, Blaze.TemplateInstance);
  if (typeof isActive === 'boolean') {
    check(isActive, Boolean);
    templateInstance.eventHookToggles.set('submit', isActive);
  }
  return templateInstance.eventHookToggles.equals('submit',true);
};

Forms.changeEventIsActive = function (templateInstance, isActive) {
  check(templateInstance, Blaze.TemplateInstance);
  if (typeof isActive === 'boolean') {
    check(isActive, Boolean);
    templateInstance.eventHookToggles.set('change', isActive);
  }
  return templateInstance.eventHookToggles.equals('change',true);
};
